      ...
      static get styles() {
        return [
          ...
          css`
            ...
            /***** Variant: filled *****/
            :host([variant='filled']) .input-group {
              background-color: ${backgroundColor};
            }

            :host([variant='filled']) .input-group::after {
              display: block;
              position: absolute;
              content: '';
              bottom: 0;
            ...
            :host([variant='filled']) ::slotted([slot='input']) {
              background-color: ${backgroundColor};
            }

            /* focused */
            :host([variant='filled'].state-focused) .input-group::after {
              transform: scale(1);
            }

            :host([variant='filled'].state-filled) .form-field__label,
            :host([variant='filled'].state-focused) .form-field__label {
              transform: translateY(-90%) translateX(-15%) scale(0.7);
            }

            /**** Variant: outlined ****/
            :host([variant='outlined']) .input-group {
              background: none;
              border: 1px solid #000;
              border-radius: 4px;
            }

            :host([variant='outlined']) ::slotted([slot='input']) {
              background: none;
            }

            /* focused */
            :host([variant='outlined'].state-filled) .form-field__label,
            :host([variant='outlined'].state-focused) .form-field__label {
              transform: translateY(-140%) translateX(-5%) scale(0.7);
              color: var(--primary-color);
              background: #fff;
            }
          `,
